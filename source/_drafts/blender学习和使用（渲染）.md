...
1. 为了让物体的阴影显示出来，在物体的底部创建一个足够涵盖所有阴影大小的平面。选中这个平面，在平面的 `Object Properties>Visibility` 下勾选 `Shadow Catcher` 。这样可以让物体的阴影显示到平面上，同时不会让底部的平面影响最终的出图效果。要注意的是，开启了 `Shadow Catcher` 之后的平面设置材质将不会有效果。
...
这里的纯黑材质指的是在任意光照条件下都不反光的材质，效果类似于下面这种：

![black_material](blender学习和使用（渲染）/black_material.png)

在这张示例图中，四个顶点上的球体被设置成不反光的纯黑。表示模型的边的线框修改器结果被设置成普通的黑色材质。

这种材质在创建的时候不选择 `Principled BSDF` ，选择 `Diffuse BSDF` ，创建之后再调整到纯黑的颜色即可，除了纯黑的颜色，也可以设置成别的纯色。
